...
export default function KeyringSettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-paper px-5 pt-4 pb-16">
      {/* Header */}
      ...
      {/* Eyebrow */}
      <p className="mb-1 font-mono text-[9px] tracking-[0.16em] text-stamp-deep">
        SCOREBO · KEYRING
      </p>
      <h2 className="mb-4 font-serif text-[22px] font-bold text-ink">あなたの鍵</h2>

      {!hasActiveKey ? (
        <div className="rounded-xl border border-dashed border-rule bg-paper-warm px-5 py-8 text-center">
          <p className="mb-1 text-[14px] font-semibold text-ink">鍵がまだありません</p>
          <p className="mb-4 text-[12px] text-ink-mute">鍵を作ると、別の端末でも記録を戻せます。</p>
          <Link
            href="/keys/new"
            className="inline-block rounded-xl bg-stamp px-5 py-2.5 text-[13px] font-bold text-white"
          >
            鍵をつくる →
          </Link>
        </div>
        ...
        <>
          {/* Key card */}
          <div className="relative mb-4 rounded-xl border border-stamp/20 bg-white p-4">
            <div className="absolute top-3 right-3">
              <div
                className="flex h-[42px] w-[42px] items-center justify-center rounded-full bg-stamp-soft text-[18px] font-serif font-bold text-stamp"
            ...
            <p className="mb-0.5 font-mono text-[9px] tracking-[0.12em] text-stamp">
              ACTIVE · KEY
            </p>
            ...
            {revealedKey ? (
              <>
                <div className="mb-1 font-mono text-[16px] font-medium tracking-[0.04em] text-ink">
                  <span className="text-[12px] text-stamp">SB-</span>
                  {revealedKey.slice(3)}
                </div>
                <div className="mb-3 flex justify-center py-1">
                  <QRCodeSVG value={revealedKey} size={100} />
                </div>
                <p className="mb-2 text-center font-mono text-[9px] text-ink-mute">
                  30秒後に非表示になります
                </p>
              </>
            ) : (
            ...
            <p className="mb-3 font-mono text-[10.5px] text-ink-mute">
            ...
            <div className="flex gap-1.5">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={btn.action}
                  className="flex-1 rounded-lg border border-rule-soft bg-paper-warm px-1.5 py-1.5 text-[11px] font-semibold text-ink"
                >
                  {btn.label}
                </button>
              ))}
          ...
          {/* Save methods */}
          <p className="mb-2 font-mono text-[9.5px] tracking-[0.12em] text-stamp-deep">
            この鍵をどこに保存したか
          </p>
              ...
              ([key, { icon, label }]) => {
                ...
                return (
                  <button
                    ...
                    className="grid grid-cols-[28px_1fr_auto] items-center gap-2 rounded-lg border border-rule-soft bg-white px-3 py-2"
                  >
                    <span className="text-[16px]">{icon}</span>
                    <span className={`text-left text-[12px] font-medium ${done ? "text-ink" : "text-ink-mute"}`}>
                      {label}
                    </span>
                    <span className={`text-[11px] font-mono ${done ? "text-stamp" : "text-ink-mute"}`}>
                      {done ? "✓ 保存済" : "未保存"}
                    </span>
                  </button>
                );
          ...
          {/* Tip */}
          <div className="mb-6 rounded-lg border border-dashed border-stamp/40 bg-paper-warm px-3 py-2.5">
        ...
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/30 px-4 pb-8">
          <div className="w-full max-w-[440px] rounded-2xl bg-white p-5 shadow-xl">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowRegenModal(false)} className="flex-1 rounded-lg border border-rule py-2.5 text-[13px] font-semibold text-ink">
                キャンセル
              </button>
              <button type="button" onClick={handleRegenerate} disabled={loading === "regen"} className="flex-1 rounded-lg bg-stamp py-2.5 text-[13px] font-bold text-white disabled:opacity-50">
      ...
      {showRevokeModal && (
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/30 px-4 pb-8">
          <div className="w-full max-w-[440px] rounded-2xl bg-white p-5 shadow-xl">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowRevokeModal(false)} className="flex-1 rounded-lg border border-rule py-2.5 text-[13px] font-semibold text-ink">
                キャンセル
              </button>
