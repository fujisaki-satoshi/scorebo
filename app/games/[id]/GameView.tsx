...
function ShareModal({ game, onClose }: { game: Game; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    setImageUrl(
      `${window.location.origin}/games/${game.id}/opengraph-image?v=${v}`,
    );
    if (typeof navigator !== "undefined" && typeof navigator.share === "function") {
      setCanNativeShare(true);
    }
  }, [game.id, game.updated_at, game.created_at]);
  ...
  async function handleNativeShare() {
    ...
    try {
      ...
      if (canShareFiles && file) {
        await navigator.share({ title: shareTitle, text: shareText, url, files: [file] });
